...
export default {
  findAll(req, res, next) {
    ...
    if (filter) {
      query.item = {
        $regex: filter, // RE so it will get every invoice that has the filter
                        // value whether char or the exact match
      };
    }
    if (sortField && sortDir) {
      options.sort = {
        [sortField]: sortDir, // sort the field by ascending or descending
      };
    }
    console.log(options);
    // were able to call paginate bc we installed the packed and connected it
    // in invoice model
