...
export class ClientListingComponent implements OnInit {
  ...
  deleteBtnHandler(clientId) {
    this.clientService.deleteClient(clientId)
      .subscribe(data => {
        // "remove" is from lodash - probably can do this with a 
        // filter as well instead of lodash
  ...
  openDialog(clientId: string): void {
    ...
    // debugger; (to create break point to debug code)

    // allows us to display the user values in the inputs when the
    // user wants to edit a client
    let dialogRef = this.dialog.open(FormDialogComponent, options);

    // this event will be called when the dialog is closed
    // the form value will be send from the dialog in form-dialog.component.html
    dialogRef.afterClosed()
      .filter(clientParam => typeof clientParam === 'object') // makes sure the client is created only when the user hits "save" and not "cancel" since its a close event
      .flatMap(result => {
      ...
      .subscribe(client => {
        ...
        if (clientId) {
          ...
          this.dataSource.data.push(client); // to make the newly created client
                                             // appear immediately in client table
                                             // w/o having to refresh the page after
                                             // its created
